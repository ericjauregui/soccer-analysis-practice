...
#######
pm["named_pass1"] = (
    pm["clearance"] + pm["cross"] + pm["assist"] + pm["keypass"]
)
...
# b
dfp_bra2 = dfp.query("team == 'Brazil'")[
...
df_comb2 = pd.concat(
    [
        df_name.set_index(["player_id", "match_id"]),
...
for group in ["A", "B", "C", "D", "E", "F", "G", "H"]:
    (
        dft.query(f"grouping == '{group}'").to_csv(
            path.join(DATA_DIR, f"dft_{group}.csv"), index=False
        )
    )
...
df = pd.concat(
    [
        pd.read_csv(path.join(DATA_DIR, f"dft_{group}.csv"))
        for group in ["A", "B", "C", "D", "E", "F", "G", "H"]
    ],
    ignore_index=True,
)
